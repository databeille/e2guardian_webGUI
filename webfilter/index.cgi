...
#		LSCONTENT=$(echo $LSCONTENT)
		...
		LSCONTENT=`echo "$LSCONTENT" | sed 's/index\.cgi"/index\.cgi?"/g'`
#		LSCONTENT=`echo "$LSCONTENT" | sed 's/index\.cgi?/index\.cgi?logs=stats\&/g'`
#		LSCONTENT=`echo "$LSCONTENT" | sed 's/logs=stats//g'`
		
		## replaces references to a direct cgi script
		set -- index topsites month_detail year_detail whousesite day_detail group_detail user_detail user_month user_year bigfiles graph user_time get
		while [ $# -gt 0 ]
			do
			[ "$1" = "get" ] && 
			{	
